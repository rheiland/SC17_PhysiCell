...
for lval in [15, 60, 120]:  # from bottom-to-top (Y-axis)
  ...
  for rval in [0.033, 0.2, 1]:  # from left-to-rightop (X-axis)
    ...
    # hmm, guess not guaranteed that these 2 arrays will have same len; luckily they do here.
    metric_vals = num_live_cancer_cells_above_threshold[idx]/num_live_cancer_cells[idx]
...
title_string = "Avg(#cells>threshold/#cells) per N runs; Migration Bias=%s" % (migration_bias)
...
png_filename = "heatmap_avg_cells_threshold_ratio_bias%s.png" % (migration_bias)
print("---> ", png_filename)
